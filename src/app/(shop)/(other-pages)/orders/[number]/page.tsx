...
export async function generateMetadata({ params }: { params: Promise<{ number: string }> }): Promise<Metadata> {
  ...
  if (!order) {
    return {
      ...
      description: 'ההזמנה שאתה מחפש לא קיימת.',
    }
  }
    ...
    <div className="container">
      <div className="mx-auto max-w-2xl py-16 sm:py-24 lg:max-w-7xl">
        <div className="space-y-4 sm:flex sm:items-end sm:justify-between sm:space-y-0 sm:px-0">
          <div>
            <Text className="mb-1">
              <time dateTime="2025-03-22">מרץ 22, 2025</time>
            </Text>
            <Heading level={1} className="text-2xl font-bold tracking-tight text-zinc-900 sm:text-3xl">
              <span data-slot="italic">הזמנה</span> #{order.number}
            </Heading>
          </div>
            ...
            {products.map((product) => (
              <div key={product.id} className="border-t border-b border-zinc-200 bg-white sm:rounded-lg sm:border">
                <div className="px-4 py-6 sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:p-8">
                  ...
                  <div className="mt-6 lg:col-span-5 lg:mt-0">
                    <dl className="grid grid-cols-2 gap-x-6 text-sm">
                      ...
                      <div>
                        <dt className="font-medium text-zinc-900 uppercase">עדכוני משלוח</dt>
                ...
                <div className="border-t border-zinc-200 px-4 py-6 sm:px-6 lg:p-8">
                  <h4 className="sr-only">סטטוס הזמנה</h4>
                  ...
                  <div aria-hidden="true" className="mt-6">
                    ...
                    <div className="mt-6 hidden grid-cols-4 text-sm font-medium text-zinc-500 uppercase sm:grid">
                      <div className="text-zinc-950">הזמנה נשלחה</div>
                      <div className={clsx(product.step > 0 ? 'text-zinc-950' : '', 'text-center')}>מעבד</div>
                      <div className={clsx(product.step > 1 ? 'text-zinc-950' : '', 'text-center')}>משולם</div>
                      <div className={clsx(product.step > 2 ? 'text-zinc-950' : '', 'text-right')}>משולם</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Billing */}
        <div className="mt-16">
          <h2 className="sr-only">סיכום חשבונית</h2>
          ...
          <div className="bg-zinc-50 px-4 py-6 sm:rounded-lg sm:px-6 lg:grid lg:grid-cols-12 lg:gap-x-8 lg:px-8 lg:py-8">
            <dl className="grid grid-cols-2 gap-6 text-sm uppercase sm:grid-cols-2 md:gap-x-8 lg:col-span-7">
              <div>
                ...
                <dd className="mt-3 text-zinc-500">
                  <span className="block">איתי קונימבו</span>
                  <span className="block">כביש הפרדס 12</span>
                  <span className="block">פתח תקווה, ישראל</span>
                </dd>
              </div>
              <div>
                <dt className="font-medium text-zinc-900">פרטי תשלום</dt>
                <dd className="-mt-1 -mr-4 flex flex-wrap">
                  <div className="mt-4 mr-4 shrink-0">
                    ...
                    <p className="sr-only">ויזה</p>
                  </div>
                  <div className="mt-4 mr-4">
                    <p className="text-zinc-900">4 ספרות אחרונות 4242</p>
            ...
            <dl className="mt-8 space-y-5 text-sm uppercase lg:col-span-5 lg:mt-0">
              <div className="flex items-center justify-between">
                <dt className="text-zinc-600">סה"כ</dt>
                <dd className="font-medium text-zinc-900">$72</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-zinc-600">משלוח</dt>
                <dd className="font-medium text-zinc-900">$5</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-zinc-600">מס</dt>
                <dd className="font-medium text-zinc-900">$6.16</dd>
              </div>
              <Divider />
              <div className="flex items-center justify-between">
                <dt className="font-medium text-zinc-900">סה"כ</dt>
