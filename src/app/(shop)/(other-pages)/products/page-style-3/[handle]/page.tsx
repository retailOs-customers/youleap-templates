...
export default async function Product({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const product = await getProductByHandle('demo-product-3')
  // for demo product. you need to change for the yourself
  const relatedProducts = (await getCollections('fashion'))?.[0].products
  // for demo product. you need to change for the yourself
          ...
          <TabPanels className="mt-10 lg:mt-16">
            <TabPanel>
              <ProductDetailContent
                ...
                imageSrc={'/images/fashion/feature-1.png'}
              />
            </TabPanel>
            <TabPanel>
